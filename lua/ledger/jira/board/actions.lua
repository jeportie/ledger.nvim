...
function M.refresh()
  ...
  refetch_issues(function(err)
    if err then
      notify("refresh failed: " .. tostring(err), vim.log.levels.ERROR)
      return
    end
    vim.schedule(function()
      if get_window().is_open() then
        get_window().rerender()
      end
    end)
  end)
  ...
  if st.me and st.me.accountId then
    ...
    api.get_myself(function(me, err)
      if err or not me then
        notify("whoami failed: " .. tostring(err), vim.log.levels.ERROR)
        return
      end
  ...
  status_picker.open(issue.key, cur_status, function(to_name, err)
    ...
    vim.schedule(function()
      ...
      refetch_issues(function(rerr)
        if rerr then
          notify("refresh failed: " .. tostring(rerr), vim.log.levels.ERROR)
          return
        end
        vim.schedule(function()
          if get_window().is_open() then
...
function M.assign_selected_to_me()
  ...
  ensure_me(function(me)
    api.set_assignee(issue.key, me.accountId, function(_, err)
      if err then
        notify("assign failed: " .. tostring(err), vim.log.levels.ERROR)
        return
      end
...
function M.unassign_selected()
  ...
  api.set_assignee(issue.key, nil, function(_, err)
    if err then
      notify("unassign failed: " .. tostring(err), vim.log.levels.ERROR)
      return
    end
...
function M.toggle_epic_at_cursor()
  ...
  local s = win._state
  local built = s.built
  if not (built and built.bands) then
    return
  end
