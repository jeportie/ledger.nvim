...
-- A set { [step_id] = true } for every step whose `match` substring appears in
-- the current process listing. `runner` (injectable for tests) returns that
-- listing as a string.
function M.running_steps(steps, runner)
  local listing = (runner or default_runner)() or ""
  local out = {}
  for _, step in ipairs(steps or {}) do
    if step.match and step.match ~= "" and listing:find(step.match, 1, true) then
      out[step.id] = true
    end
  end
  return out
